...
- La página Recursos funciona como biblioteca de la edición: reúne únicamente registros con `status: publish` y comunica su relación opcional con unidad y clase. Los atributos semánticos de las tarjetas permiten incorporar filtros sin duplicar el listado.
- Los filtros de Recursos se generan en el navegador desde las tarjetas publicadas y permiten combinar tipo, unidad, clase y etiqueta. El listado conserva su lectura completa si JavaScript no está disponible.
- La acción de cada recurso depende de `delivery`. Los archivos propios descargables se publican bajo `assets/downloads/` y utilizan el atributo HTML `download`; los destinos externos o embebidos conservan acciones de apertura. Un recurso `publish` no debe ofrecer una acción cuyo destino no exista.
- El cuerpo Markdown de una clase es su fuente canónica. El visor transforma en el navegador los bloques separados por `<!-- page -->` y muestra uno a la vez; sin JavaScript conserva la lectura vertical. El visor puede maximizarse mediante la API nativa de pantalla completa, sin incluir las actividades o recursos posteriores. Los archivos PDF o PPT se importan a este formato en lugar de utilizarse como fuente primaria.
